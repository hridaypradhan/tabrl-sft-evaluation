...
def main():
    ...
    print(f"[info] running SFT1 schema induction on {len(prompts)} examples", file=sys.stderr)
    ...
    for doc_id, out in zip(ids, outputs):
        ...
        if schema_obj is None or not isinstance(schema_obj, dict):
            ...
            print(f"[warn] doc {doc_id}: failed to parse JSON; saving raw only", file=sys.stderr)
            continue
        ...
        if not columns:
            # if the model produced something weird, treat as failure
            n_fail += 1
            raw_path = out_dir / f"{doc_id}.raw_schema.txt"
            raw_path.write_text(gen, encoding="utf-8")
            print(f"[warn] doc {doc_id}: no usable columns; saving raw only", file=sys.stderr)
            continue
        ...
        out_path.write_text(json.dumps(final_schema, ensure_ascii=False, indent=2), encoding="utf-8")
        n_ok += 1

    print(f"[info] SFT1 done. Parsed {n_ok} schemas, {n_fail} failures.", file=sys.stderr)
