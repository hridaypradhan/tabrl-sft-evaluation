...
def load_livesum_with_schemas(test_file: str, schema_dir: str):
    ...
    for ex in data:
        ...
        samples.append({"id": doc_id, "text": ex["text"], "schema_str": schema_str})
    if not samples:
...
def main():
    ...
    ap.add_argument("--schema_dir", required=True, help="schemas/livesum_sft1_vllm_pi2")
    ap.add_argument("--out_jsonl", required=True, help="tables/livesum_sft2_vllm_pi2.jsonl")
    ...
    print(f"[info] running SFT2 on {len(prompts)} Livesum examples", file=sys.stderr)
